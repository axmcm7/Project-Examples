...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        mapped = operands.map(lambda operand: scheme_eval(operand, env))
        return scheme_apply(self, mapped, env)     
...
def do_or_form(expressions, env):
    ...
    while expressions:
        ...
        val = scheme_eval(expressions.first, env, end)
        if scheme_truep(val):
            return val
        expressions = expressions.second
    return False
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            check_procedure(procedure)
            args = rest.map(lambda operand: scheme_eval(operand, env))
            #result = procedure.eval_call(args, env)
